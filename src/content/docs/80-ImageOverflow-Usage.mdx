...
1. 在 Image 组件旁边添加 "ImageOverflow" 组件。请注意，如果你还没有为 Image 组件分配精灵，"ImageOverflow" 脚本将不起作用。

   （图片：ImageOverflow 组件）

2. 要调整锚点，点击启用锚点编辑模式的按钮。要禁用锚点编辑模式，可以再次点击此按钮。

   （图片：锚点编辑模式按钮）

3. - **1-4** — 用于调整边距的 gizmo 方块。
...
4. 调整边距后的示例。正如我们所见，阴影（也是精灵的一部分）延伸到了 RectTransform 边界之外。

   （图片：边距调整示例）

5. 在 HorizontalLayoutGroup 内的 Image 组件使用 'ImageOverflow' 的示例。
